...
class Meteor:
    ...
    def _check_process(self):
        """Check if the METEOR process is still alive."""
        if self.meteor_p is None or self.meteor_p.poll() is not None:
            # Process is dead or not started, restart it
            self._restart_process()

    def _restart_process(self):
        """Restart the METEOR process."""
        with self.lock:
    ...
    def _stat(self, hypothesis_str, reference_list):
        ...
        try:
            ...
            logging.warning("BrokenPipeError in METEOR, restarting process...")
            self._restart_process()
            ...
            logging.error(f"Error writing to METEOR process: {e}")
            self._restart_process()
            # Retry once after restart
            self.meteor_p.stdin.write(enc(score_line))
            self.meteor_p.stdin.write(enc('\n'))
